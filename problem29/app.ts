...
function getADashNumbers(limitNumber: number): { base: number; exponent: number }[] {
  const aDashNumbers: { base: number; exponent: number }[] = [];
  const tmpArr: number[] = [];
  for (let i = 2; i <= Math.sqrt(limitNumber); i++) {
    for (let j = 1; Math.pow(i, j) <= limitNumber; j++) {
      ...
      if (!tmpArr.includes(tmpValue)) {
        ...
        aDashNumbers.push({ base: i, exponent: j });
      }
    }
  }
  return aDashNumbers;
...
function countDistinctPowers(numberA: number, numberB: number): number {
  ...
  for (let i = 0; i < aDashNumbersLength; i++) {
    ...
    for (let b = 2; b <= numberB; b++) {
      ...
      const tmpValue = `${aDash.base}^${aDash.exponent * b}`;
  ...
  const ansCount = distinctValues.size + (numberA - 1 - aDashNumbersLength) * (numberB - 1);
  // const ansCount = distinctValues.length + (numberA - 1 - aDashNumbersLength) * (numberB - 1);
  return ansCount;
}

console.log(`2 ≤ a ≤ 100 と 2 ≤ b ≤ 100の範囲でa^bが異なる値となる数は${countDistinctPowers(10000, 10000)}です。`);
