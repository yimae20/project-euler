...
function calc(numerator: number[], denominator: number[]): number {
  ...
  for (let i = 0; i < numeratorLength; i++) {
    ...
    numeratorPrimes = [...numeratorPrimes, ...primes];
  }
  const numeratorPrimesLength = numeratorPrimes.length;
  for (let i = 0; i < denominatorLength; i++) {
    const primes = factorizationInPrimeNumbers(denominator[i]);
    denominatorPrimes = [...denominatorPrimes, ...primes];
  }
  ...
  let ansNumerator = 1;
  for (let i = 0; i < numeratorPrimesLength; i++) {
    ansNumerator *= numeratorPrimes[i];
  }
  let ansDenominator = 1;
  for (let i = 0; i < denominatorPrimesLength; i++) {
    ansDenominator *= denominatorPrimes[i];
  }
